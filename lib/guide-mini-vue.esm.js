...
function transformChildren(node, context) {
    return () => {
        if (node.type === "element" /* ENodeType.ELEMENT */) {
            const { children } = node;
            const length = children.length;
            /**
             * children
            ...
            if (length === 1) {
                ...
                if (child.type === "element" /* ENodeType.ELEMENT */) {
                ...
                for (let i = 0; i < children.length; i++) {
                    const child = children[i];
                    if (isText(child) || child.type === "compound_expression" /* ENodeType.COMPOUND_EXPRESSION */) {
...
export { createApp, createVNode as createElementVNode, createRenderer, createTextVNode, getCurrentInstance, h, inject, nextTick, provide, reactive, ref, registerCompiler, renderSlots, toDisplayString };
