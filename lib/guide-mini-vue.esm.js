...
function createRenderer(options) {
    ...
    function patchKeyedChildren(c1, c2, container, parent, anchor) {
        ...
        if (i > e1) {
            ...
            let increasingNewIndexSubsequence = getSequence(newIndexToOldIndexMap);
            let j = increasingNewIndexSubsequence.length - 1;
            for (let i = toBePatched - 1; i >= 0; i--) {
                if (i !== increasingNewIndexSubsequence[j]) {
                    /**
                     * 交换位置
...
export { createApp, createRenderer, createTextVNode, getCurrentInstance, h, inject, provide, reactive, ref, renderSlots };
